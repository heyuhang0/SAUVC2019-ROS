...
class BaseStrategy:
    def __init__(self):
        ...
        rospy.Publisher('/bubbles_pid/roll/setpoint', Float64, queue_size=10).publish(Float64(0))
        rospy.Publisher('/bubbles_pid/pitch/setpoint', Float64, queue_size=10).publish(Float64(0))
        self._yaw_pub = rospy.Publisher('/bubbles_pid/yaw/setpoint', Float64, queue_size=10)
        self._depth_pub = rospy.Publisher('/bubbles_pid/depth/setpoint', Float64, queue_size=10)
        self._speed_pub = rospy.Publisher('/bubbles_pid/speed/setpoint', Float64, queue_size=10)

        self._yaw_first_update = True
        rospy.Subscriber('/bubbles_state/yaw', Float64, self._on_yaw_update)
        rospy.Subscriber('/bubbles_cv/gate/pos', BoundingBox2D, self._on_gate_pos_update)
        rospy.Subscriber('/bubbles_cv/flare/pos', BoundingBox2D, self._on_flare_pos_update)
